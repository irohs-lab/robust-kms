...
# SPSA implemented her is not used for attack
...
def pgd_attack_on_ker(X_train, Ms, sols, bandwidth,
 ...
 #       ensure we can take grad w.r.t. X_adv
    ...
    #            loss = F.mse_loss(out, y)               # squared-error
        ...
        adversary=AutoAttack(forw, norm=AA_NORM, eps=epsilon, version='standard')
        ...
        breakpoint()
    elif pgd == False and bb_attack == "square":
        # def model(x): return forw(x.flatten(1))
        # adversary = AutoAttack(model, norm='Linf', eps=eps, version='standard')
        # adversary.attacks_to_run = ['square']
        # adversary.square.n_queries=1000
        # x = X_nat.reshape(X_nat.shape[0], 3, 32, 32)
        # x_adv = adversary.run_standard_evaluation_individual(x,y,bs=X_test.shape[0],return_labels=False)["square"]
        # # return x_adv
        def model(x): 
        # x comes in as (N, 3, 32, 32) from Square attack
        # flatten it to (N, 3072) for your kernel
            return forw(x.flatten(1))
    
        adversary = AutoAttack(model, norm='Linf', eps=eps, version='standard')
        adversary.attacks_to_run = ['square']
        adversary.square.n_queries=1000
        
        # Ensure X_nat is in correct 4D shape before passing to Square
        if X_nat.ndim == 2:  # if it's flattened (N, 3072)
            x = X_nat.reshape(X_nat.shape[0], 3, 32, 32)
        else:  # already (N, 3, 32, 32)
            x = X_nat
        
        X_adv = adversary.run_standard_evaluation_individual(
            x, y, bs=X_test.shape[0], return_labels=False
        )["square"]
...
#        spsa_sample_size = 100  # or set to a fixed number like 128
        if epsilon > 0:
            X_adv = spsa(
                loss_fn,
                X_adv,
