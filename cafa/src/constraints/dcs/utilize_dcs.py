...
# Added new method to get violated DCs for a given sample, which can be used for debugging and analysis of the constraints.    
    def get_violated_dcs(self, sample: np.ndarray):
        """
        Return detailed information about which denial constraints are violated
        by a single sample.

        IMPORTANT:
        - `sample` must already be in the constrainer-space format
        (i.e. the same format expected by `check_sat`).
        - This checks DC violations against each DC's stored `other_tuples_data`,
        consistent with the rest of the CAFA pipeline.

        Returns:
            violated_info: list[dict]
                Each dict contains:
                - dc_idx: index within self.dcs
                - dc_file_idx: original index from mined DC file if available
                - dc_repr: string form of the denial constraint
                - violating_pair_count: how many "other tuples" violate this DC
                - violating_pair_indices: indices (local to dc.other_tuples_data) of violating pairs
        """
        sample = np.asarray(sample, dtype=np.float32)

        assert len(sample) == len(self.feature_names), (
            f"Sample length {len(sample)} does not match number of features {len(self.feature_names)}"
        )

        # Build the target tuple dict expected by DenialConstraint
        target_tuple = {
            feature_name: sample[f_idx]
            for f_idx, feature_name in enumerate(self.feature_names)
        }

        violated_info = []

        for dc_idx, dc in enumerate(self.dcs):
            # is_sat_arr[j] == True means target_tuple satisfies this DC with other_tuple j
            is_sat_arr, sat_predicates_count_arr = dc.check_satisfaction_all_pairs(target_tuple)

            violating_pair_indices = np.where(~is_sat_arr)[0]

            if len(violating_pair_indices) > 0:
                violated_other_tuples = dc.other_tuples_data.iloc[violating_pair_indices].to_dict(orient="records")

                violated_info.append({
                    "dc_idx": int(dc_idx),
                    "dc_file_idx": int(dc.dc_file_idx) if dc.dc_file_idx is not None else None,
                    "dc_repr": str(dc),
                    "violating_pair_count": int(len(violating_pair_indices)),
                    "violating_pair_indices": violating_pair_indices.tolist(),
                    "violating_other_tuples_preview": violated_other_tuples[:3],
                })
