        ...
        for step in tqdm(range(self.max_iter_tabpgd)):
            ...
            if self.random_init and step == 0:
                ...
                if not np.isfinite(grad).all():
                    print(f"[step={step}] bad grad detected")
                    print("nan count:", np.isnan(grad).sum())
                    print("inf count:", np.isinf(grad).sum())
                    bad_rows = np.where(~np.isfinite(grad).all(axis=1))[0]
                    print("bad rows (first 10):", bad_rows[:10])
            ...
            frozen_diff = np.abs(x_adv - x_adv_before_perturb)[~mask.astype(bool)]
            if frozen_diff.size > 0:
                print("max frozen-feature diff:", frozen_diff.max())
